    ...
    axios.get(`/book/getLibraries/${userId}`)
      .then(response => {
        ...
        libs.forEach(library => {
          axios.get(`/book/getSpecLibrary/${library.libId}/id/${userId}`)
            .then(res => {
              // Recupera i dettagli di ciascun libro utilizzando un'altra API
              const bookDetailsPromises = res.data.map(book => {
                return axios.get(`https://example-data.draftbit.com/books?id=${book.bookId}`)
                  .then(bookDetailsResponse => {
                    // Aggiungi 'pagesRead' ai dettagli del libro
                    return { ...bookDetailsResponse.data, pagesRead: book.pagesRead };
                  })
                  .catch(err => {
                ...
                .then(completedBooks => {
                  ...
                  setLibraries(prevLibraries =>
                    prevLibraries.map(lib =>
                      lib.libId === library.libId ? { ...lib, books: completedBooks.filter(book => book != null) } : lib
                    )
                  );
                });
            });
