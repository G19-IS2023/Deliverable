...
router.get('/getUser/:userId', verifyToken, async(req: Request, res: Response) => {
    ...
    try{
        ...
        const user = await db?.collection("users").findOne( { _id: new ObjectId(objectId) } ) as User | null;
...
router.delete("/deleteProfile/:userId", verifyToken, async (req: Request, res: Response) => {
    ...
    try {
        ...
        await db?.collection('users').deleteOne({ _id: new ObjectId(objectId) });
